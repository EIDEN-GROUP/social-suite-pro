...
function StoryRing({ children, accent, active }: { children: React.ReactNode; accent: string; active?: boolean }) {
  return (
    <span
...
function Instagram({ company, posts, highlights, onTap, onHighlight }: { company: Company; posts: Post[]; highlights: Highlight[]; onTap?: (p: Post) => void; onHighlight?: (h: Highlight) => void }) {
    ...
    <div className="text-sm">
      ...
      <div className="flex items-center gap-4 px-3 py-3">
        ...
        <div className="flex flex-1 justify-around text-center text-xs">
          <div><div className="text-base font-semibold">{gridPosts.length}</div>posts</div>
          <div><div className="text-base font-semibold">{company.followers || "0"}</div>followers</div>
          <div><div className="text-base font-semibold">{company.following || 0}</div>following</div>
        </div>
      </div>
      <div className="px-3 pb-3">
        ...
        {company.link && <div className="mt-0.5 text-xs font-medium" style={{ color: accent }}>{company.link.replace(/^https?:\/\//, "")}</div>}
      </div>

      {highlights.length > 0 && (
        <div className="flex gap-3 overflow-x-auto px-3 pb-3">
          {highlights.map((h) => (
            <button key={h.id} onClick={() => onHighlight?.(h)} className="flex w-14 shrink-0 flex-col items-center gap-1">
              <span className="grid h-12 w-12 place-items-center overflow-hidden rounded-full p-[2px]" style={{ boxShadow: "inset 0 0 0 2px rgba(0,0,0,0.12)" }}>
      ...
      <div className="flex border-t border-foreground/10">
        <button onClick={() => setTab("posts")} className={`flex flex-1 justify-center border-b-2 py-2 ${tab === "posts" ? "border-foreground text-foreground" : "border-transparent text-muted-foreground"}`}><Grid3x3 className="h-4 w-4" /></button>
        <button onClick={() => setTab("reels")} className={`flex flex-1 justify-center border-b-2 py-2 ${tab === "reels" ? "border-foreground text-foreground" : "border-transparent text-muted-foreground"}`}><Film className="h-4 w-4" /></button>
        <button className="flex flex-1 justify-center border-b-2 border-transparent py-2 text-muted-foreground"><UserCheck className="h-4 w-4" /></button>
      </div>
          ...
          {Array.from({ length: Math.max(list.length, minCells) }).map((_, i) => {
            ...
            return (
              ...
              >
                ...
                {p?.post_type === "reel" && <Play className="absolute right-1 top-1 h-3 w-3 fill-white text-white drop-shadow" />}
                {p?.post_type === "carousel" && <Images className="absolute right-1 top-1 h-3 w-3 text-white drop-shadow" />}
              </button>
            );
          })}
        </div>
...
function BizHeader({ company, variant }: { company: Company; variant: "facebook" | "twitter" | "linkedin" }) {
  ...
  const bannerStyle = (gradient: string) => (cover ? { backgroundImage: `url(${cover})` } : { background: gradient });
      ...
      <div className="bg-background">
        ...
        <div className="px-3">
          <div className="-mt-6 flex items-end justify-between">
            <span className="rounded-full ring-4 ring-background"><Avatar company={company} size={48} /></span>
            <button className="mb-1 rounded-full bg-foreground px-3 py-1 text-[11px] font-semibold text-background">Follow</button>
          </div>
          ...
          <div className="mt-1 flex gap-3 pb-2 text-[11px]"><span><b>{following}</b> <span className="text-muted-foreground">Following</span></span><span><b>{followers}</b> <span className="text-muted-foreground">Followers</span></span></div>
        </div>
      </div>
    );
  }
  if (variant === "facebook") {
    return (
      <div className="bg-background">
        <div className="h-14 bg-cover bg-center" style={bannerStyle(`linear-gradient(135deg, ${accent}, ${accent}88)`)} />
        <div className="px-3 pb-2">
          <span className="-mt-6 block w-fit rounded-full ring-4 ring-background"><Avatar company={company} size={56} /></span>
          <div className="mt-1 text-base font-bold">{company.name}</div>
          <div className="text-[11px] text-muted-foreground">{company.category || "Business"} · {followers} followers</div>
          {company.bio && <div className="mt-1 text-xs">{company.bio}</div>}
          {linkText && <div className="text-[11px] font-medium" style={{ color: accent }}>{linkText}</div>}
          <div className="mt-2 flex gap-2">
            <button className="flex-1 rounded py-1 text-xs font-semibold text-white" style={{ background: accent }}>＋ Follow</button>
            <button className="flex-1 rounded bg-foreground/5 py-1 text-xs font-semibold">Message</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-background">
      <div className="h-12 bg-cover bg-center" style={bannerStyle(`linear-gradient(135deg, ${accent}, ${accent}88)`)} />
      <div className="px-3 pb-2">
        ...
        <div className="mt-2 flex gap-2">
          <button className="rounded-full px-4 py-1 text-xs font-semibold text-white" style={{ background: accent }}>＋ Follow</button>
          <button className="rounded-full border border-foreground/30 px-4 py-1 text-xs font-semibold">Website</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="text-sm">
      <div className="flex flex-col items-center px-3 pt-3">
        ...
        <div className="mt-2 flex gap-5 text-center text-xs">
          <div><div className="font-bold">{following}</div><div className="text-muted-foreground">Following</div></div>
          <div><div className="font-bold">{followers}</div><div className="text-muted-foreground">Followers</div></div>
          <div><div className="font-bold">{posts.length}</div><div className="text-muted-foreground">Videos</div></div>
        </div>
        <button className="mt-2 rounded px-8 py-1 text-xs font-semibold text-white" style={{ background: accent }}>Follow</button>
        {company.category && <div className="mt-1 text-xs text-muted-foreground">{company.category}</div>}
        {company.bio && <div className="mt-0.5 text-center text-xs">{company.bio}</div>}
      </div>
      {posts.length === 0 ? <Empty /> : (
        <div className="mt-3 grid grid-cols-3 gap-0.5">
          {posts.map((p) => (
            <button key={p.id} onClick={() => onTap?.(p)} className={`relative aspect-[9/14] overflow-hidden bg-black ${statusRing(p.status)}`}>
              <Media post={p} className="h-full w-full object-cover" />
      ...
      {posts.slice(0, 8).map((p) => (
        <button key={p.id} onClick={() => onTap?.(p)} className={`flex w-full gap-2 py-3 text-left ${statusRing(p.status)} rounded`}>
          ...
          <div className="flex-1">
            ...
            {p.media_url && <Media post={p} className="mt-2 rounded-xl border border-foreground/10" controls />}
